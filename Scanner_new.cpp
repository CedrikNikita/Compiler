...
Scanner :: Lexem Scanner :: peek(){
	...
	while(state != C_END){
		char c = file.get();
		if(c == -1) return Lexem(x, temp_y + 1, "", "", 0);
		temp_y++;
		if(c == '\n'){
			x++;
			temp_y = 0;
		}
		prev_state = table[state][c < -1 ? 128 : c];
		text += c;
		value += c;
		lex_size++;
		if(prev_state == C_BEGIN){
			value = "";
			text = "";
			lex_size = 0;
			value_size = 0;
		...
		char temp = file.peek();
		if(temp == -1) break;
		state = table[prev_state][temp < -1 ? 128 : temp];
	}
	if(lex_size == 0 && file.peek() == -1) return Lexem(x, temp_y + 1, "", "", 0);
	int base = 0;
	switch(prev_state){
...
Error :: Error(Scanner :: Lexem _Lex, bool a): Lex(_Lex), end(1){
	...
	if(prev_state == C_CHAREND || prev_state == C_CHARCHECK)
		str = "missing terminating ' character";
	if(prev_state == C_STRING)
		str = "missing terminating \" character";		
	if(prev_state == C_STRINGSPECIAL || prev_state == C_CHARSPECIAL){
		str = "unknown escape sequence ";
		end = 2;
